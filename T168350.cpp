...
const int P=65536;const int SF=16;const int msk=65535;ll mod=(int)1e9+7;ll PP;
typedef long double ld;const ld pi=acos(-1.0);int tt,nn,kk;int ff[1000001];
...
}inline void poly_mul(ll* a,ll* b,ll* c,int len,int d)
{
...
//inline int FastFact(int n)
//{
//	iter::ih();
//    int bl=sqrt(n);PP=(ll)P*P%mod;solve(bl);ll res=1;
//    for(int i=0,id=0;;i+=bl,id++) 
//    {
...
//    return res;
//}
#define FastFact(n) ff[n]
inline long long FastPow(long long b,long long p)
{
    if(p==0)
    return 1%mod;
    long long t=FastPow(b,p/2);
    t=(t%mod)*(t%mod)%mod;
    if(p%2==1)
    t=(t%mod)*(b%mod)%mod;
    return t;
...
int main()
{
	pre();
	ff[1]=1;
    for(int i=2;i<=1000000;i++)ff[i]=ff[i-1]*i%mod;
    cin>>tt;
    while(tt--)
    {
    	cin>>nn>>kk;
    	if(kk==0){cout<<"1\n";continue;}
    	if(kk>log2(nn)){cout<<FastFact(nn)<<'\n';continue;}
    	int part1=nn,part2=nn-1,cnt1=1,cnt2=0;
    	for(int i=1;i<=kk;i++)
    	{
    		int A,B,C,D;
    		if(part1&1)A=part1/2+1,B=part1/2;
    		else A=part1/2,B=part1/2-1;
    		if(part1&1)C=cnt1,D=cnt1+cnt2*2;
    		else C=cnt1*2+cnt2,D=cnt2;
    		part1=A;part2=B;cnt1=C;cnt2=D;
		}
//		cout<<nn<<' '<<part1<<' '<<part2<<' '<<cnt1<<' '<<cnt2<<' ';
		cout<<FastFact(nn)/FastPow(FastFact(part1),cnt1)/FastPow(FastFact(part2),cnt2)<<'\n';
	}
}
